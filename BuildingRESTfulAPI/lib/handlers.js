  ...
  if(id){
    ...
    _data.read('checks',id,function(err,checkData){
      if(!err && checkData){
        ...
        // get the token from the headers
    var token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
    //verify that the header token is valid and belongs to user that created check
...
// Checks put
// required data: id
// optional data: protocol, url, method, successCodes,timeoutSeconds (one must be included/sent)
handlers._checks.put = function(data,callback){
  ...
  if(id){
    // check that one optional field is included
      ...
      _data.read("checks",id, function(err,checkData){
        if(!err && checkData){
          ...
          if(tokenIsValid){
            ...
            if(successCodes){
              checkData.sucessCodes = successCodes;
            }
            if(timeoutSeconds){
                ...
                _data.read('users',checkData.userPhone,function(err,userData){
                  if(!err && userData){
                    ...
                    if(checkPosition > -1){
                      ...
                      callback(500,{"Error":"Could not find check on user object, so could not delete check"});
                    }

              } else{
                callback(500,{"Error":"Could not find the use that created the check, so could not remove the check from list of checks on user object"});
              }
            });
